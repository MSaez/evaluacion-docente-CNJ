...
class TblUserController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if (isset($_POST['TblUser'])) {
			...
			$model->password=md5($model->password);//para que guarde en md5
			if ($model->save()) {
	...
	public function actionUpdate($id)
	{
		...
		if (isset($_POST['TblUser'])) {
			$model->attributes=$_POST['TblUser'];
			$model->password=md5($model->password);//para guardar en md5
			if ($model->save()) {
				$this->redirect(array('view','id'=>$model->id));
			}
		}
	...
	public function actionDelete($id)
	{
		if (Yii::app()->request->isPostRequest) {
			// we only allow deletion via POST request
              ...
              Yii::app()->user->setFlash(TbHtml::ALERT_COLOR_SUCCESS, "Se ha borrado correctamente.");	
             }
             catch(CDbException $e){
              		
              		$this->redirect(isset($_POST['returnUrl']) ? $_POST['returnUrl'] : array('admin'));     
              		Yii::app()->user->setFlash(TbHtml::ALERT_COLOR_ERROR, "<strong>Error!</strong> Existen evaluaciones registradas.");
            }
		} else {
